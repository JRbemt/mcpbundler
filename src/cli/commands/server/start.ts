...
import { startDaemon, getDaemonStatus, getLogPaths } from '../../utils/daemon.js';
...
export async function startCommand(options: StartOptions): Promise<void> {
  ...
  if (options.daemon) {
    ...
    try {
      // Dynamic import to avoid executing main.ts during CLI parsing
      const mainModule = await import('../../../main.js');
      // Main module will handle startup
    } catch (error) {
      logger.error({ error }, 'Failed to start server');
      process.exit(1);
    }
