...
/**
 * Configure pm2-logrotate module settings
 */
async function configurePM2LogRotate(): Promise<void> {
  const settings = {
    "pm2-logrotate:max_size": "10M",
    "pm2-logrotate:retain": "10",
    "pm2-logrotate:compress": "true",
    "pm2-logrotate:dateFormat": "YYYY-MM-DD_HH-mm-ss",
  };

  for (const [key, value] of Object.entries(settings)) {
    await new Promise<void>((resolve, reject) => {
      pm2.set(key, value, (err) => {
        if (err) reject(err);
        else resolve();
      });
    });
...
export async function startDaemon(port: number, databaseUrl?: string): Promise<DaemonStatus> {
  ...
  return withPM2Connection(async () => {
    // Configure pm2-logrotate before starting the process
